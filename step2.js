...
Step2.resetConstFileUpload = function() {
	...
	cell.textContent = "";
	var changeLink = window.document.getElementById("changePassedConstFile");
	changeLink.onclick = Step2.resetConstFileUpload;
	window.document.getElementById("passedDescription").style.display = "none";
	var constFile = window.document.getElementById("constFile");
	constFile.style.display = "block";
	var textField = window.document.getElementById("completePassedConstFileContent");
	textField.value = "";
};
Step2.setConstFileContent = function(str) {
	var displayStr = "" + str;
	if(str.length > 15) {
		displayStr = HxOverrides.substr(str,0,12) + "...";
	}
	window.document.getElementById("passedSpan").style.display = "block";
	var cell = window.document.getElementById("passedConstFileContent");
	cell.textContent = displayStr;
	var changeLink = window.document.getElementById("changePassedConstFile");
	changeLink.onclick = Step2.resetConstFileUpload;
	window.document.getElementById("passedDescription").style.display = "block";
	var constFile = window.document.getElementById("constFile");
	constFile.style.display = "none";
	var textField = window.document.getElementById("completePassedConstFileContent");
	textField.value = str;
};
Step2.clearURI = function() {
	var tmp = window.history;
	var tmp1 = window.document.title;
	tmp.replaceState({ },tmp1,window.location.pathname);
};
Step2.resetForm = function() {
	var textField = window.document.getElementById("phaseOutFile");
	textField.value = "";
	Step2.resetConstFileUpload();
	var textField2 = window.document.getElementById("constFile");
	textField2.value = "";
	var checkbox = window.document.getElementById("sortSequences");
...
Step2.run = function(sort,reduce,outFile,constFile) {
	var result = SeqPhase2.parse(outFile,constFile).getFasta(reduce,sort);
	window.setTimeout(function() {
		Step2.handleResults(result);
	},0);
};
Step2.runReadConstFile = function(sort,reduce,outFileContent) {
	var constFile = window.document.getElementById("constFile");
	var passedConstFileContentField = window.document.getElementById("completePassedConstFileContent");
	if(passedConstFileContentField.value != "") {
		var constFileContentField = window.document.getElementById("completePassedConstFileContent");
		...
		window.setTimeout(function() {
			Step2.run(sort,reduce,outFileContent,"");
		},0);
...
Step2.runReadOutFile = function(sort,reduce) {
	var outFileInput = window.document.getElementById("phaseOutFile");
...
Step2.runJob = function() {
	var sortE = window.document.getElementById("sortSequences");
	var sort = sortE.checked;
	var reduceE = window.document.getElementById("reduceSequences");
...
Step2.main = function() {
	...
	if(uri != null && uri != "") {
		if(StringTools.startsWith(uri,"?")) {
			uri = HxOverrides.substr(uri,1,null);
		}
		var parts = uri.split("&");
		var _g = 0;
		...
		while(_g < _g1) {
			...
			if(StringTools.startsWith(parts[i],"constFileContent=")) {
				var constFileContent = StringTools.trim(HxOverrides.substr(parts[i],17,null)).toUpperCase();
				if(constFileContent != null && constFileContent != "") {
					Step2.setConstFileContent(constFileContent);
					Step2.clearURI();
				}
			}
		}
	}
	window.document.getElementById("resetJob2Button").onclick = Step2.resetForm;
	window.document.getElementById("runJob2Button").onclick = Step2.runJob;
};
