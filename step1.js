...
SeqPhase1Result.prototype = {
	...
	,hasAlignErrors: function(i) {
		if(i == 1) {
			return this.hasAlign1Errors();
		} else if(i == 2) {
			return this.hasAlign2Errors();
		} else if(i == 3) {
			return this.hasAlign2Errors();
		}
		return false;
	}
	,getAlignErrors: function(i) {
		if(i == 1) {
			return this.getAlign1Errors();
		} else if(i == 2) {
			return this.getAlign2Errors();
		} else if(i == 3) {
			return this.getAlign3Errors();
		}
		return null;
	}
	,hasAlignWarn: function(i) {
		if(i == 1) {
			return this.hasAlign1Warn();
		} else if(i == 2) {
			return this.hasAlign2Warn();
		} else if(i == 3) {
			return this.hasAlign3Warn();
		}
		return false;
	}
	,getAlignWarn: function(i) {
		if(i == 1) {
			return this.getAlign1Warn();
		} else if(i == 2) {
			return this.getAlign2Warn();
		} else if(i == 3) {
			return this.getAlign3Warn();
		}
		return null;
	}
...
var Step1 = function() { };
Step1.__name__ = true;
Step1.errFunction = function(msg,url,line,col,error) {
	window.alert("Error " + msg + ", line " + line);
	return null;
};
Step1.resetForm = function() {
	var textField1 = window.document.getElementById("alig1");
	textField1.value = "";
	var textField2 = window.document.getElementById("alig2");
	textField2.value = "";
	var textField3 = window.document.getElementById("alig3");
	textField3.value = "";
	window.document.getElementById("align1FileErrorsArea").style.display = "none";
	window.document.getElementById("align1FileWarningsArea").style.display = "none";
	window.document.getElementById("align2FileErrorsArea").style.display = "none";
	window.document.getElementById("align2FileWarningsArea").style.display = "none";
	window.document.getElementById("align3FileErrorsArea").style.display = "none";
	window.document.getElementById("align3FileWarningsArea").style.display = "none";
	window.document.getElementById("generalErrorsArea").style.display = "none";
	window.document.getElementById("generalWarningsArea").style.display = "none";
	window.document.getElementById("notesArea").style.display = "none";
	window.document.getElementById("outFileResult").style.display = "none";
	window.document.getElementById("knownFileResult").style.display = "none";
	window.document.getElementById("constFileResult").style.display = "none";
	window.document.getElementById("suggestedCommandArea").style.display = "none";
	window.document.getElementById("proceedArea").style.display = "none";
};
Step1.copySuggestedCommand = function() {
	var copyText = window.document.getElementById("suggestedCommandField");
	copyText.select();
...
Step1.resetResultArea = function() {
	...
	window.document.getElementById("align" + 1 + "FileWarningsList").innerHTML = "";
	window.document.getElementById("align" + 2 + "FileErrorsArea").style.display = "none";
	window.document.getElementById("align" + 2 + "FileErrorsList").innerHTML = "";
	window.document.getElementById("align" + 2 + "FileWarningsArea").style.display = "none";
	window.document.getElementById("align" + 2 + "FileWarningsList").innerHTML = "";
	window.document.getElementById("align" + 3 + "FileErrorsArea").style.display = "none";
	window.document.getElementById("align" + 3 + "FileErrorsList").innerHTML = "";
	window.document.getElementById("align" + 3 + "FileWarningsArea").style.display = "none";
	window.document.getElementById("align" + 3 + "FileWarningsList").innerHTML = "";
	window.document.getElementById("generalErrorsArea").style.display = "none";
	window.document.getElementById("generalErrorsList").innerHTML = "";
	window.document.getElementById("generalWarningsArea").style.display = "none";
	window.document.getElementById("generalWarningsList").innerHTML = "";
	window.document.getElementById("notesArea").innerHTML = "";
	window.document.getElementById("outFileResult").style.display = "none";
	window.document.getElementById("knownFileResult").style.display = "none";
	window.document.getElementById("constFileResult").style.display = "none";
	window.document.getElementById("suggestedCommandArea").style.display = "none";
...
Step1.handleResults = function(result) {
	Step1.resetResultArea();
	var _g = 1;
	while(_g < 4) {
		var k = _g++;
		if(result.hasAlignErrors(k)) {
			window.document.getElementById("align" + k + "FileErrorsArea").style.display = "block";
			var errors = result.getAlignErrors(k);
			var _g1 = 0;
			var _g2 = errors.length;
			while(_g1 < _g2) {
				var i = _g1++;
				var ele = window.document.createElement("li");
				ele.innerText = errors[i];
				window.document.getElementById("align" + k + "FileErrorsList").append(ele);
			}
		}
		if(result.hasAlignWarn(k)) {
			window.document.getElementById("align" + k + "FileWarningsArea").style.display = "block";
			var warnings = result.getAlignWarn(k);
			var _g3 = 0;
			var _g4 = warnings.length;
			while(_g3 < _g4) {
				var i1 = _g3++;
				var ele1 = window.document.createElement("li");
				ele1.innerText = warnings[i1];
				window.document.getElementById("align" + k + "FileWarningsList").append(ele1);
			}
		}
	}
	if(result.hasGeneralErrors()) {
		window.document.getElementById("generalErrorsArea").style.display = "block";
		...
		while(_g < _g1) {
			...
			var ele = window.document.createElement("li");
			ele.innerText = errors[i];
			window.document.getElementById("generalErrorsList").append(ele);
		}
	}
	if(result.hasGeneralWarn()) {
		window.document.getElementById("generalWarningsArea").style.display = "block";
		var warnings = result.getGeneralWarn();
		var _g = 0;
		var _g1 = warnings.length;
		while(_g < _g1) {
			var i = _g++;
			var ele = window.document.createElement("li");
			ele.innerText = warnings[i];
			window.document.getElementById("generalWarningsList").append(ele);
		}
	}
	if(result.hasNotes()) {
		window.document.getElementById("notesArea").style.display = "block";
		var notes = result.getNotes();
		var _g = 0;
		var _g1 = notes.length;
		while(_g < _g1) {
			var i = _g++;
			var ele = window.document.createElement("br");
			window.document.getElementById("notesArea").append(ele);
			var ele1 = window.document.createElement("span");
			ele1.innerText = notes[i];
			window.document.getElementById("notesArea").append(ele1);
		}
	}
	if(result.hasSuggestedCommand()) {
		window.document.getElementById("suggestedCommandArea").style.display = "block";
		var textField1 = window.document.getElementById("suggestedCommandField");
		textField1.value = result.getSuggestedPhaseCommand();
	}
	if(result.hasInpFile()) {
		window.document.getElementById("outFileResult").style.display = "block";
		var link = window.document.getElementById("downloadInpFileLink");
		var b64 = window.btoa(result.getInpFile());
		link.href = "data:text/plain;base64,\n" + b64;
	}
	if(result.hasKnownFile()) {
		window.document.getElementById("knownFileResult").style.display = "block";
		var link = window.document.getElementById("downloadKnownFileLink");
		var b64 = window.btoa(result.getKnownFile());
		link.href = "data:text/plain;base64,\n" + b64;
	}
	if(result.hasConstFile()) {
		window.document.getElementById("constFileResult").style.display = "block";
		var link = window.document.getElementById("downloadConstFileLink");
		var b64 = window.btoa(result.getConstFile());
		link.href = "data:text/plain;base64,\n" + b64;
	}
	if(result.hasInpFile()) {
		window.document.getElementById("proceedArea").style.display = "block";
		if(result.hasConstFile()) {
			var ele = window.document.getElementById("finalLink");
...
Step1.run = function(align1Content,align2Content,align3Content) {
	var result = SeqPhase1.doIt(align1Content,align2Content,align3Content);
	window.setTimeout(function() {
		Step1.handleResults(result);
	},0);
};
Step1.runReadAlign3 = function(align1Content,align2Content) {
	var fObj = window.document.getElementById("alig3");
	if(fObj.files.length == 1) {
		var reader = new FileReader();
		reader.onload = function(data) {
			var fileContent = data.target.result;
			window.setTimeout(function() {
				Step1.run(align1Content,align2Content,fileContent);
			},0);
		};
		reader.readAsText(fObj.files[0],"ISO-8859-1");
	} else {
		window.setTimeout(function() {
			Step1.run(align1Content,align2Content,null);
		},0);
	}
};
Step1.runReadAlign2 = function(align1Content) {
	var fObj = window.document.getElementById("alig2");
	if(fObj.files.length == 1) {
		var reader = new FileReader();
		reader.onload = function(data) {
			var fileContent = data.target.result;
			window.setTimeout(function() {
				Step1.runReadAlign3(align1Content,fileContent);
			},0);
		};
		reader.readAsText(fObj.files[0],"ISO-8859-1");
	} else if(align1Content == null) {
		window.alert("Missing input data! Either an alignment of sequences from homozygous individuals and from heterozygotes to be phased or an alignment of fake haplotype pairs from heterozygotes to be phased has to be provided. You can also provide both files!");
	} else {
		window.setTimeout(function() {
			Step1.runReadAlign3(align1Content,null);
		},0);
	}
};
Step1.runReadAlign1 = function() {
	var fObj = window.document.getElementById("alig1");
	if(fObj.files.length == 1) {
		var reader = new FileReader();
		reader.onload = function(data) {
			var fileContent = data.target.result;
			window.setTimeout(function() {
				Step1.runReadAlign2(fileContent);
			},0);
		};
		reader.readAsText(fObj.files[0],"ISO-8859-1");
	} else {
		window.setTimeout(function() {
			Step1.runReadAlign2(null);
		},0);
	}
};
Step1.main = function() {
	window.onerror = Step1.errFunction;
	window.document.getElementById("resetJob1Button").onclick = Step1.resetForm;
	window.document.getElementById("copySuggestedCommand").onclick = Step1.copySuggestedCommand;
	window.document.getElementById("submitJob1Button").onclick = Step1.runReadAlign1;
};
